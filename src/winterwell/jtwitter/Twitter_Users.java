 ...
 * Use {@link Twitter#users()} to get one of these objects.
 * <p>
...
public class Twitter_Users {
	...
	List<User> bulkShow2(String apiMethod, Class stringOrNumber, Collection screenNamesOrIds) {
		int batchSize = 100;
		ArrayList<User> users = new ArrayList<User>(screenNamesOrIds
				.size());
		...
		for (int i = 0; i < _screenNamesOrIds.size(); i += batchSize) {
			...
			try {
				String json = http.getPage(jtwit.TWITTER_URL + apiMethod,
	 ...
	 *
	 * @param user
	 * @return fresh user object, or null if (a) they were already being followed,
	...
	public boolean isFollower(String followerScreenName,
		...
		try {
			...
			String whoFirst = followedScreenName.equals(jtwit.getScreenName())? followerScreenName : followedScreenName;
			try {
			...
			if (e.getMessage()!=null && e.getMessage().contains(
				...
				throw new TwitterException("WTF? inputs: follower="+
						...
						followedScreenName+", call-by="+jtwit.getScreenName()+"; "+e.getMessage());
			}
			throw e;
		}
	}
	...
	public User show(String screenName) throws TwitterException, TwitterException.SuspendedUser {
		...
		String json = http.getPage(jtwit.TWITTER_URL + "/users/show.json", vars, http
				.canAuthenticate());
		http.updateRateLimits(KRequestType.SHOW_USER);
		if (json.length() == 0)
	...
	public List<User> searchUsers(String searchTerm) {
		...
		String json = http.getPage(jtwit.TWITTER_URL + "/users/search.json", vars,
				true);
	...
	public List<User> getFollowers(String username) throws TwitterException {
		return getUsers(jtwit.TWITTER_URL + "/statuses/followers.json", username);
	}
	...
	public User notify(String username) {
		...
		String page = http.getPage(jtwit.TWITTER_URL + "/notifications/follow.json",
				vars, true);
		try {
			return new User(new JSONObject(page), null);
		} catch (JSONException e) {
	...
	public User stopFollowing(String username) {
		...
		try {
			Map<String, String> vars = InternalUtils.asMap("screen_name", username);
			page = jtwit.http.post(jtwit.TWITTER_URL + "/friendships/destroy.json", vars,
					true);
	...
	public List<User> getRelationshipInfo(List<String> screenNames) {
		if (screenNames.isEmpty()) return Collections.EMPTY_LIST;
		List<User> users = bulkShow2("/friendships/lookup.json", String.class, screenNames);
		return users;
	}
	...
	public List<User> getRelationshipInfoById(List<? extends Number> userIDs) {
		if (userIDs.isEmpty()) return Collections.EMPTY_LIST;
		List<User> users = bulkShow2("/friendships/lookup.json", Number.class, userIDs);
		return users;
	}
